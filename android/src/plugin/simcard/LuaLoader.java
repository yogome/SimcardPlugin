...
// e.g. [Lua] require "plugin.simcard"
...
public class LuaLoader implements JavaFunction, CoronaRuntimeListener {
	...
	public int invoke(LuaState L) {
		...
		NamedJavaFunction[] luaFunctions = new NamedJavaFunction[] {
			new webViewWrapper(),
			new getTelephonyData(),
			new enableWifi(),
			new enableMobileData(),
		};
	 ...
	 */
	public int showWebView(LuaState L) {
		...
		final String paramUrl = L.checkString( 1 );
		...
		if (activity != null) {
			activity.runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					CoronaActivity currentActivity = CoronaEnvironment.getCoronaActivity();
					Intent intent = new Intent(currentActivity, ExtendedWebView.class);
	...
	private class getTelephonyData implements NamedJavaFunction{
		@Override
		public String getName(){
			return "getTelephonyData";
		}

		@Override
		public int invoke(LuaState L){
			SimCard simcard = new SimCard();
			Hashtable<String, String> simInfo = simcard.getData();
			L.newTable(0, simInfo.size());

			int luaTableStackIndex = L.getTop();
		
			// Copy the Java hashtable's entries to the Lua table.
			for (java.util.Map.Entry<String, String> entry : simInfo.entrySet()) {
				...
				L.setField(luaTableStackIndex, key);
			}
	...
	private class webViewWrapper implements NamedJavaFunction {
		/**
		...
		@Override
		public int invoke(LuaState L) {
			return showWebView(L);
		}
	}

	private class enableWifi implements NamedJavaFunction {
		/**
		 * Gets the name of the Lua function as it would appear in the Lua script.
		 * @return Returns the name of the custom Lua function.
		 */
		...
		public int invoke(LuaState L) {
			...
			final Boolean isWifi = L.checkBoolean( 1 );
	...
	private class enableMobileData implements NamedJavaFunction {
		/**
		 * Gets the name of the Lua function as it would appear in the Lua script.
		 * @return Returns the name of the custom Lua function.
		 */
		...
		public int invoke(LuaState L) {
			...
			final Boolean isMobile = L.checkBoolean( 1 );
			final CoronaActivity activity = CoronaEnvironment.getCoronaActivity();
			...
			try{
	            ...
	            setMobileDataEnabledMethod.invoke(connectivityManager, isMobile);
