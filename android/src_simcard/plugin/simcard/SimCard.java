...
public class LuaLoader implements JavaFunction, CoronaRuntimeListener {
	...
	public int getData(LuaState L) {
		...
		if (activity != null) {
            ...
            if (param.equals("DeviceId")){
                returnValue = tm.getDeviceId();
                L.pushString(returnValue);
            }
            else if (param.equals("DeviceSoftwareVersion")){
            	
                returnValue = tm.getDeviceSoftwareVersion();
                L.pushString(returnValue);
            }
            else if (param.equals("NetworkOperator")){
            	
                returnValue = tm.getNetworkOperator();
                L.pushString(returnValue);
            }
            else if (param.equals("NetworkOperatorName")){
            	
            	returnValue = tm.getNetworkOperatorName();
                L.pushString(returnValue);
            }
            else if (param.equals("SimOperator")){
            	
            	returnValue = tm.getSimOperator();
                L.pushString(returnValue);
            }
            else if (param.equals("SimSerialNumber")){
            	
            	returnValue = tm.getSimSerialNumber();
                L.pushString(returnValue);
            }
            else if (param.equals("SimState")){
            	
            	returnValue = Integer.toString(tm.getSimState());
            	L.pushString(returnValue);
            }
            else if (param.equals("isSMSCapable")){
            	
            	returnValue = Boolean.toString(tm.isSmsCapable());
                L.pushString(returnValue);
            }
         }
		return 1;
	...
	private class getSimcardInfo implements NamedJavaFunction {
		/**
