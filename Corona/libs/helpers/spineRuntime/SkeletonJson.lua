...
local SkeletonData = require( folder.."SkeletonData" )
local BoneData = require( folder.."BoneData" )
local SlotData = require( folder.."SlotData" )
local Skin = require( folder.."Skin" )
local AttachmentLoader = require( folder.."AttachmentLoader" )
local Animation = require( folder.."Animation" )
local IkConstraintData = require( folder.."IkConstraintData" )
local IkConstraint = require( folder.."IkConstraint" )
local EventData = require( folder.."EventData" )
local Event = require( folder.."Event" )
...
function SkeletonJson.new(attachmentLoader)
	...
	function self:readSkeletonData(jsonText)
		...
		-- Bones.
		for index = 1, #root["bones"] do --
			local boneMap = root["bones"][index]
			...
			skeletonData.bones[#skeletonData.bones + 1] = boneData
		end
		...
		if root["slots"] then
			for i = 1, #root.slots do
				...
				skeletonData.slots[#skeletonData.slots + 1] = slotData
		...
		if root["skins"] then
			for skinName,skinMap in pairs(root["skins"]) do
				...
				if skin.name == "default" then
					...
					skeletonData.skins[#skeletonData.skins + 1] = skin
				end
			end
		end
			...
			for slotName,timelineMap in pairs(slotsMap) do
				...
				for timelineName,values in pairs(timelineMap) do
					if timelineName == "color" then
						...
						duration = mathMax(duration, timeline:getDuration())
				...
				for timelineName,values in pairs(timelineMap) do
					...
					elseif timelineName == "flipX" or timelineName == "flipY" then
						...
						for index = 1, #values do
							timeline:setFrame(frameIndex, values[index]["time"], values[index][field] or false)
							frameIndex = frameIndex + 1
						end
						timelines[#timelines + 1] = timeline
						duration = mathMax(duration, timeline:getDuration())
						...
						for i,valueMap in ipairs(values) do
							...
							if not valueMap["vertices"] then
								...
								if self.scale == 1 then
									...
									for ii = 1, #verticesValue do
										vertices[ii + start] = verticesValue[ii] * self.scale
									end
								end
								if isMesh then
									local meshVertices = attachment.vertices
									for ii = 1 + start, vertexCount do
