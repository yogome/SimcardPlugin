...
function SkeletonBounds.new()
	...
	local function aabbCompute()
		...
		for index = 1, #polygons do
			local count = #polygons[index]
			for ii = 1, count, 2 do
				local x = polygons[index][ii]
				local y = polygons[index][ii + 1]
				minX = mathMin(minX, x)
				minY = mathMin(minY, y)
				maxX = mathMax(maxX, x)
				maxY = mathMax(maxY, y)
	...
	function self:update(skeleton, updateAabb)
		...
		for index = 1, #skeleton.slots do
			local boundingBox = skeleton.slots[index].attachment
			if boundingBox and boundingBox.type == AttachmentType.boundingbox then
				...
				boundingBox:computeWorldVertices(x, y, skeleton.slots[index].bone, polygon)
			end
		end
