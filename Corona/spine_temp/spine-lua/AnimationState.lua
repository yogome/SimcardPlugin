...
function AnimationState.new (data)
	...
	function self:addAnimationByName (trackIndex, animationName, loop, delay, options)
		local animation = self.data.skeletonData:findAnimation(animationName)
		if not animation then error("Animation not found: " .. animationName) end
		return self:addAnimation(trackIndex, animation, loop, delay, options)
	end

	-- Adds an animation to be played delay seconds after the current or last queued animation.
	-- @param delay May be <= 0 to use duration of previous animation minus any mix duration plus the negative delay.
	function self:addAnimation (trackIndex, animation, loop, delay, options)
		local entry = AnimationState.TrackEntry.new()
		entry.animation = animation
		entry.loop = loop
		entry.endTime = animation.duration
		...
		options = options or {}
		entry.onEnd = options.onEnd
		entry.onComplete = options.onComplete
		entry.onStart = options.onStart
