...
local IMAGESUFIX = assetPath .. "geoAirport1_"
local COUNTRIES = {"brazil","canada","colombia","france","japan","mexico","philippines","russia","spain","uk","usa","argentina"}
local POSCOUNTRIES = {
...
local LINECOLORS = {
	[1] = { r = 1, g = 99/255, b = 252/255},
	[2] = { r = 1, g = 251/255, b = 0},
	[3] = { r = 1, g = 115/255, b = 0},
	[4] = { r = 0, g = 98/255, b = 1},
	[5] = { r = 141/255, g = 0, b = 222/255},
	[6] = { r = 158/255, g = 0, b = 0},
	[7] = { r = 0, g = 158/255, b = 13/255},
	[8] = { r = 0, g = 0, b = 0},
}
...
local function createWrongGroup()
	...
	local pivotX, pivotY = -120 , -50
	for counter = 1, #tableToCheck do
		local country = display.newImage(IMAGESUFIX .. tableToCheck[counter] .. ".png")
		country.x, country.y = pivotX, pivotY
		pivotX = pivotX + 120
		if counter == 3 then
			pivotY = pivotY + 100
			pivotX = -120
		end
		local countryText = display.newText(localization.getString("country" .. tableToCheck[counter]),country.x,country.y + 30,settings.fontName,20)
		group:insert(country)
		group:insert(countryText)
	end
	group.alpha = 0
	return group
end
...
local function onTouched(event)
	...
	if "began" == phase then
		local indexToUse = mRandom(#LINECOLORS)
		colorToUse.r = LINECOLORS[indexToUse].r
		colorToUse.g = LINECOLORS[indexToUse].g
		colorToUse.b = LINECOLORS[indexToUse].b
...
local function createAnswers()
	...
	local pivot = 100
	local tempParts = extraTable.deepcopy(COUNTRIES)
	for counter = 1, NUMBEROFANSWERS do
		local plane = display.newImage(assetPath .. "plane.png")
		plane.index = counter
		plane.x, plane.y = flyBoard.x + pivot, antartic.y - 50
...
function game:create(event) -- This will be fired the first time you load the scene. If you deload the scene this can get called again. The best practice is to make your scenes without memory leaks.
	...
	countriesGroup = display.newGroup()

	local backSea = display.newImage(IMAGESUFIX .. "background.png")
	backSea.x, backSea.y = centerX, centerY
	backSea.width, backSea.height = screenWidth, screenHeight
	backgroundLayer:insert(backSea)
	
	local backMap = display.newImage(IMAGESUFIX .. "map.png")
	backMap.x, backMap.y = centerX, centerY
	backgroundLayer:insert(backMap)
	
	antartic = display.newImage(IMAGESUFIX .. "antarctica.png")
	antartic.anchorY = 1
	antartic.x, antartic.y = centerX, screenBottom
	backgroundLayer:insert(antartic)
	
	flyBoard = display.newImage(IMAGESUFIX .. "path.png")
	...
	departureBoard = display.newImage(IMAGESUFIX .. "departureBoard.png")
	...
	for counter = 1, #COUNTRIES do
		local country = display.newImage(IMAGESUFIX .. COUNTRIES[counter] .. ".png")
		country.x, country.y = POSCOUNTRIES[counter].x, POSCOUNTRIES[counter].y
		country.name = COUNTRIES[counter]
		countriesGroup:insert(country)
	end
	backgroundLayer: insert(countriesGroup)
